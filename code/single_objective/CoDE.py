        ...
        while self.terminated(population) == False:
            ...
            for i in range(NIND):
                ...
                u_population[0].Chrom[i] = algorithms.mutde_1(population.Chrom[r_list[0]], population.Chrom[r_list[1]],
                                                            population.Chrom[r_list[2]], self.mut_parameters_pool[parameter_index])
                u_population[0].Chrom[i] = algorithms.binary_recombination(population.Chrom[i], u_population[0].Chrom[i],
                                                                         self.bin_parameters_pool[parameter_index],
                                                                           self.population.Field[1], self.population.Field[0]
                                                                           )
                ...
                u_population[1].Chrom[i] = algorithms.mutde_2(population.Chrom[r_list[0]], population.Chrom[r_list[1]],
                                                            ...
                                                            population.Chrom[r_list[4]], self.mut_parameters_pool[parameter_index])
                u_population[1].Chrom[i] = algorithms.binary_recombination(population.Chrom[i], u_population[1].Chrom[i],
                                                                         self.bin_parameters_pool[parameter_index],
                                                                           self.population.Field[1], self.population.Field[0])

                # 使用 current-to-rand/1生成第三个差分算子
                u_population[2].Chrom[i] = algorithms.mutde_current_to_rand(population.Chrom[i], population.Chrom[r_list[0]],
                                                                          population.Chrom[r_list[1]], population.Chrom[r_list[2]],
